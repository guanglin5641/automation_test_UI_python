...
class BasePage(object):
    host = conf.get_by_name("warehouse.host")
    supplier_host = conf.get_by_name("supplier.host")
    logger = log

    def __init__(self, driver: WebDriver,path: str, use_supplier_url: bool = False,):
        self.driver = driver
        if use_supplier_url:
            self.url = self.get_supplier_url(path)
        else:
    ...
    def get_supplier_url(self, path):
        return urljoin(self.supplier_host, path)
